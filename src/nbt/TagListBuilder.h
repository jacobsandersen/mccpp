...
public:
    ...
    static std::shared_ptr<TagListBuilder> create(std::wstring name, TagType list_type);
    ...
    std::shared_ptr<TagListBuilder> add_string(std::wstring value);
...
private:
    TagListBuilder(std::wstring name, TagType list_type) : m_name(std::move(name)), m_list_type(std::move(list_type)) {}
    std::wstring m_name;
    TagType m_list_type;
