...
const std::shared_ptr<net::Connection>& Player::get_connection() const {
  return m_connection;
}
...
void Player::set_mojang_profile(
    const std::shared_ptr<MojangProfile>& mojang_profile) {
  m_mojang_profile = mojang_profile;
}

const std::shared_ptr<ClientInformation>& Player::get_client_information()
    const {
