...
namespace celerity {
BufferCompressionType BufferCompressor::detect_compression_type(
    ByteBuffer& buffer) {
  if (buffer.get_data_length() >= 2) {
    ...
    if ((first & 0x0f) == 0x08 &&
        (static_cast<uint16_t>(first) << 8 | second) % 31 == 0) {
      return ZLIB;
    }
  }
...
ByteBuffer& BufferCompressor::compress(ByteBuffer& buffer,
                                       const BufferCompressionType type) {
  ...
  if (const int deflate_init_result =
          deflateInit2(&stream, Z_DEFAULT_COMPRESSION, Z_DEFLATED, window_bits,
                       8, Z_DEFAULT_STRATEGY);
      deflate_init_result != Z_OK) {
    throw std::runtime_error("deflateInit2 failed: " +
                             std::to_string(deflate_init_result));
  }
...
ByteBuffer& BufferCompressor::decompress(ByteBuffer& buffer) {
  ...
  if (const int inflate_init_result = inflateInit2(&stream, window_bits);
      inflate_init_result != Z_OK) {
    throw std::runtime_error("inflateInit2 failed: " +
                             std::to_string(inflate_init_result));
  }
