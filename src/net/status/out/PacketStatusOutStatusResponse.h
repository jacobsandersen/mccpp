...
#ifndef CELERITY_NET_STATUS_PACKETSTATUSOUTSTATUSRESPONSE_H
#define CELERITY_NET_STATUS_PACKETSTATUSOUTSTATUSRESPONSE_H
...
class PacketStatusOutStatusResponse : public OutboundPacket {
 public:
  PacketStatusOutStatusResponse(
      std::string version_name, uint16_t protocol_version,
      uint16_t max_player_count, uint16_t current_player_count,
      std::string motd, std::string favicon_base64,
      std::map<std::string, std::string> player_sample,
      bool enforces_secure_chat, bool previews_chat)
      : OutboundPacket(0x00),
