...
void NetworkManager::start() {
  ...
  LOG(INFO) << "NetworkManager starting " << thread_count
...
void NetworkManager::start_accept() {
  ...
  m_acceptor.async_accept(
      *conn->get_socket(), [this, conn](const boost::system::error_code& err) {
        if (!err) {
          LOG(INFO)
              << "Connection established from "
              << conn->get_socket()->remote_endpoint().address().to_string();
          std::thread([this, conn]() { start_read(conn); }).detach();
...
void NetworkManager::start_read(const std::shared_ptr<Connection>& conn) {
  ...
  boost::asio::async_read(
      *conn->get_socket(), *buf, boost::asio::transfer_at_least(1),
      [this, conn, buf](const boost::system::error_code& err,
  ...
  switch (ctx.read_state) {
    case BufferReadState::ReadPacketLength: {
      ...
      if (buffer.get_data_length() < *ctx.packet_length) {
        LOG(WARNING) << "process :: read packet length :: buffer's data length "
                        "is less than packet length";
        return;
      }
      ...
      if (conn->get_compress_packets()) {
        ...
        ctx.partial_buffer->write_ubytes(
            buffer.read_ubytes(*ctx.packet_length));
    ...
    case BufferReadState::ReadDataLength: {
      ...
      if (buffer.get_data_length() < *ctx.data_length &&
          *ctx.data_length != 0) {
        LOG(WARNING) << "Remaining buffer data length is less than declared "
                        "packet data length";
        return;
      }
      ...
      ctx.partial_buffer = std::make_unique<ByteBuffer>();
      ctx.partial_buffer->write_ubytes(buffer.read_ubytes(
          *ctx.packet_length - VarInt::encoding_length(*ctx.data_length)));
    ...
    case BufferReadState::ReadPacketId: {
      ...
      ctx.packet_id =
          std::make_unique<int32_t>(ctx.partial_buffer->read_varint());
      LOG(INFO) << "process :: read packet id :: " << *ctx.packet_id
                << " (conn state = " << static_cast<int>(conn->get_state())
                << ")";
    ...
    case BufferReadState::HandlePacket: {
      ...
      if (const auto& handlers =
              *m_packet_handlers.find(conn->get_state())->second;
          !handlers.contains(*ctx.packet_id)) {
        LOG(WARNING) << "Tried to handle unknown packet " << *ctx.packet_id
                     << " in state " << static_cast<int>(conn->get_state());
      } else {
