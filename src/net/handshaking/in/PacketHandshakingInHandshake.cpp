...
void PacketHandshakingInHandshake::handle(
  ...
  switch (next_state) {
    ...
    case 3:
    default:
      LOG(WARNING) << "Connection requested next state unknown or 3 (transfer) "
                      "which is not supported. Connection closed.";
      conn->unclean_close();
      break;
  }

  LOG(INFO) << "Client requests next state: " << next_state_summary
            << ". Switching.";
