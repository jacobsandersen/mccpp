...
void PacketConfigurationInKeepAlive::handle(
  ...
  if (const int64_t payload = buffer->read_be_long();
    ...
    PacketConfigurationOutDisconnect disconnect(
        "Client responded with invalid Keep Alive");
  ...
  conn->start_new_timer(
      std::chrono::seconds(15), [weak_conn = std::weak_ptr(conn)]() {
        ...
        if (const auto locked = weak_conn.lock()) {
          if (locked->get_state() != ConnectionState::Configuration) {
            LOG(WARNING) << "About to send new configuration-state keep alive, "
                            "but connection no longer in configuration state";
            return;
          }
