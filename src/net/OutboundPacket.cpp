...
void OutboundPacket::send(const std::shared_ptr<Connection>& conn) {
  ...
  if (!conn->get_compress_packets()) {
    ...
    if (data_length < compression_threshold) {
      ...
      packed.write_varint(
          static_cast<int32_t>(data_length_length + tmp.get_data_length()));
      packed.write_varint(static_cast<int32_t>(packet_id_length + data_length));
      packed.append(tmp);
    }
  }
  ...
  boost::asio::async_write(
      *conn->get_socket(), boost::asio::buffer(*buffer),
      boost::asio::transfer_all(),
      [](const boost::system::error_code& ec, std::size_t bytes_transferred) {
        if (ec) {
          LOG(ERROR) << "OutboundPacket - boost::asio::async_write failed: "
                     << ec.message();
        } else {
          LOG(INFO) << "OutboundPacket - bytes transferred: "
                    << bytes_transferred;
        }
      });
}
